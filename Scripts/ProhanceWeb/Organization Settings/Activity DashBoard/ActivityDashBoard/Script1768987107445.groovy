...
if (WebUI.waitForElementPresent(findTestObject('Object Repository/Organization Settings/New Folder/Page_ProHance/a_WORK TIME'), 
    ...
    WebUI.waitForElementClickable(findTestObject('Object Repository/Organization Settings/New Folder/Page_ProHance/First_Container'), 
        GlobalVariable.Delay, FailureHandling.STOP_ON_FAILURE)
    ...
    if (isFileDownload.toBoolean()) {
	    ...
	    if (isDataValion.toBoolean()) {
            ...
            if (rows != null) {
                CustomKeywords.'com.prohanceapplication.webtableexcelvalidation.DataValidator.validateWebTableWithDownloadedExcel'(
                    rows //CustomKeywords.'com.prohancewebapplication.webtable.Table.webTableDataWriter'(rows)
                    )
            }
